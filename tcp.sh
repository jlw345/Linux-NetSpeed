...
#	Description: BBR4.*/5.*内核+BBR魔改版++BBRplus+Lotserver
#	Original version: 1.3.2
#	Author: 千影,cx9208
#	Blog: https://www.94ish.me/
#	Redevelopment version: 1.0.0
#	Blog: https://leven.52ourfamily.com/
#	Add: 5.*+kernel/Remove redundant kernel
#=================================================

sh_ver="1.0.0"
github="raw.githubusercontent.com/jlw345/Linux-NetSpeed/master"
...
#安装BBR内核,ELRepo仓库最新内核
installbbr_latest_kernel(){
	if [[ "${release}" == "centos" ]]; then
	  rpm --import https://${github}/ELRepo/RPM-GPG-KEY-elrepo.org
		yum install -y https://${github}/ELRepo/elrepo-release-7.el7.elrepo.noarch.rpm
		yum remove -y kernel-headers
		yum --enablerepo=elrepo-kernel install -y kernel-ml kernel-ml-headers kernel-ml-devel
		Latest_installed_kernel=`awk -F\' '$1=="menuentry " {print i++ " : " $2}' /etc/grub2.cfg | grep "0 : "`
    kernel_version=`echo "${Latest_installed_kernel}" | awk -F\( '{print $2}'|awk -F- '{print $1}'`
	else
    echo -e "${Error} 目前仅支持CentOs，其他系统暂不支持!"
    exit 1
	fi
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重启VPS后，请重新运行脚本开启${Red_font_prefix}BBR/BBR魔改版${Font_color_suffix}"
	stty erase '^H' && read -p "需要重启VPS后，才能开启BBR ELRepo最新版，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		yum install -y make gcc
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/jlw345/Linux-NetSpeed/master/bbr/centos/tcp_nanqinlang.c
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/jlw345/Linux-NetSpeed/master/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/jlw345/lotServer/master/Install.sh) install
...
start_menu(){
clear
echo && echo -e " TCP加速 一键安装修改版 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  -- Leven | leven.52ourfamily.com --
  
 ${Green_font_prefix}0.${Font_color_suffix} 升级脚本
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR4.11内核(for BBR原版/魔改版)
 ${Green_font_prefix}2.${Font_color_suffix} 安装 BBR最新主线内核(for BBR原版)
 ${Green_font_prefix}3.${Font_color_suffix} 安装 BBRplus版内核
 ${Green_font_prefix}4.${Font_color_suffix} 安装 Lotserver(锐速)内核
————————————加速管理————————————
 ${Green_font_prefix}5.${Font_color_suffix} 使用BBR原版加速
...
————————————杂项管理————————————
 ...
 ${Green_font_prefix}12.${Font_color_suffix} 清除其余内核(仅保留当前)
 ${Green_font_prefix}13.${Font_color_suffix} 退出脚本
————————————————————————————————" && echo

	check_status
	echo -e " 当前内核版本：${Green_font_prefix}${kernel_version}${Font_color_suffix}"
...
check_status(){
	...
	elif [[ `echo ${kernel_version} | awk -F'.' '{print $1}'` == "4" ]] && [[ `echo ${kernel_version} | awk -F'.' '{print $2}'` -ge 9 ]] || [[ `echo ${kernel_version} | awk -F'.' '{print $1}'` -gt 4 ]]; then
		kernel_status="BBR"
	else 
